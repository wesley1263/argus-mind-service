...
implementation detail, swappable per `PROJECT.md`), UI presentation of Confidence
...
| Decay model produces confidence swings that feel arbitrary to Students | Medium | Medium | Decay parameters validated against known Evidence/Confidence scenarios before rollout; model version recorded for explainability (AC4). |
...
- [ ] Confidence computation is a pure function of Evidence + model version, unit-tested without
      mocks per `memory/coding-standards.md` §3.

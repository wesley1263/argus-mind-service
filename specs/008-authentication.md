...
- **Owning Engine(s):** None of the five Engines. This is a **Platform** capability
  (`platform/` in CLAUDE.md's planned layout) — identity is a supporting
...
> to the glossary term **Session** (`memory/glossary.md`), which denotes a Student's bounded
> learning interaction captured by Evidence Engine. A Student may hold one Auth Token while
> participating in zero or many Sessions. Keeping these separate is deliberate — see the ubiquitous-language rule in
> `CLAUDE.md`.
...
claims, which undermines the Evidence-is-truth rule at its root: evidence attributed to the
...
- [ ] No glossary term was redefined; "Access Token"/"Refresh Token" are documented here as
      infrastructure-level, not added to `memory/glossary.md` (they are not business concepts —
      see `memory/coding-standards.md` §1).

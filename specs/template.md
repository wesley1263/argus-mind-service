...
>
> Every specification is governed by `/.ai/constitution.md`. In particular: use only terms defined
> in `/glossary/README.md` (add a glossary entry in the same change if a concept is missing —
> Article VI); if the spec requires an irreversible or cross-Engine decision, pair it with an ADR
> before implementation begins (Article VII, see `/adr/README.md`); implementation must satisfy
> `/.ai/definition-of-done.md` before it is considered complete.
...
the Student (or the platform) today that this spec changes? Ground this in `docs/vision.md` and the
relevant Engine's responsibility in `/.ai/architecture.md` §2 — a spec that can't explain its
...
`/skills/fastapi.md`. If the API is purely an internal Engine Contract (see
`/.ai/architecture.md` §4) and not externally exposed, say so explicitly.*
...
*Schema changes this spec requires, following the conventions in `/skills/postgres.md`
(schema-per-Engine, no cross-Engine foreign keys — see `/.ai/architecture.md` §4 for why). State
...
*Spec-specific completion criteria, in addition to — not instead of — `/.ai/definition-of-done.md`,
...
- [ ] Any new business concept used here has a `/glossary/README.md` entry.

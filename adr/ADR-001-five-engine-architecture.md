...
Engine's internals. See `CLAUDE.md (Architecture section)` for the full contract table and the enforced module
dependency graph.
...
- **New obligation — this boundary must be actively defended:** nothing about a monolith's file
  ...
  be enforced in review (`.claude/agents/reviewer.md` §2) and, once `src/` exists, by import-boundary
  tooling.
...
- **No Engine boundaries — a single undifferentiated application.** Rejected: this directly
  ...
  (see `PROJECT.md`). Without enforced boundaries, Ingestion's internal
